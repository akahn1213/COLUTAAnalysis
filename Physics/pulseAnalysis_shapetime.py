...
def runAnalysis(data, adc, channel):
  ...
  for gain in available_gains:
    ...
    #print(np.shape(samples_to_save[gain][adc][channel]))
...
def save_blocks(f, s, b, attr_vals, combos):
  global n_adcs
  global n_channels
  global block_length
  ...
  for adc in range(n_adcs):
    for channel in range(n_channels):
      #print(channel)
      sample_data = []; bit_data = []
      for combo in combos:
        combo_key = combo
        sample_data.append(s[combo_key][adc][channel])
        #print('here')
        #print(combo)
        #print(s[combo_key][adc][channel])
        bit_data.append(b[combo_key][adc][channel])
        if adc == 0 and channel == 0:
          for i in range(len(b[combo_key][adc][channel])):
            for attr in attr_vals.keys():
              attr_setting = [str(attr_vals[attr][i]) for i in range(len(attr_vals[attr])) if str(attr_vals[attr][i]) in combo_key]
              attr_list[attr].append(attr_setting[0])
              #print(attr_list[attr])
      #f.create_dataset("gain_ratio_"+combo[0]+"/shape_"+combo[1]+"/coluta"+str(adc+1)+"/channel"+str(channel+1)+"/samples", data=s[adc][channel])
      #f.create_dataset("gain_ratio_"+combo[0]+"/shape_"+combo[1]+"/coluta"+str(adc+1)+"/channel"+str(channel+1)+"/bits", data=b[adc][channel])
      makeAttributes(f, attr_list, combos)
      sample_data_ar = np.concatenate(sample_data, axis = 0)
      bit_data_ar = np.concatenate(bit_data, axis = 0)
      f.create_dataset("coluta"+str(adc+1)+"/channel"+str(channel+1)+"/samples", data=sample_data_ar)
      f.create_dataset("coluta"+str(adc+1)+"/channel"+str(channel+1)+"/bits", data=bit_data_ar)
      #f["/adc"+str(adc)].create_dataset("samples", data = s[adc])
      #f["/adc"+str(adc)].create_dataset("bits", data = b[adc])
  #f.close()

def get_data_from_hdf5(f, measurements):
  global n_adcs
  global n_channels
  global n_samples_per_run
  global pulse_length
  data = np.empty(shape=(n_adcs, n_channels, 2, len(measurements), n_samples_per_run))
  ...
  for adc in range(n_adcs):
    for channel in range(n_channels):
      ...
      for measurement in measurements:
        ...
        data[adc][channel][0][meas_counter] = f["Measurement_"+str(measurement)+"/coluta"+str(adc+1)+"/channel"+str(channel+1)+"/samples"][()][n_pulse_skips*pulse_length - extra_sample_offset:n_pulse_skips*pulse_length - extra_sample_offset + n_samples_per_run]
        data[adc][channel][1][meas_counter] = f["Measurement_"+str(measurement)+"/coluta"+str(adc+1)+"/channel"+str(channel+1)+"/bits"][()][n_pulse_skips*pulse_length - extra_sample_offset:n_pulse_skips*pulse_length - extra_sample_offset + n_samples_per_run].astype(int)
        meas_counter += 1
  return data

def makeAttributes(f, attr_vals, combos):
  global block_length
  global adc_freq
  global awg_freq
  global n_adcs
  ...
  f.attrs.create("attribute_combos", combos)
  for attr in attr_vals.keys():
    f.attrs.create(attr, attr_vals[attr])
...
if __name__ == "__main__":
  ...
  for run in runs:
    ...
    for amp in amps:
      ...
      saveFile = makeH5File(saveDir+'Blocks_Amp'+amp+'_'+gain+'.hdf5', 'core')
      tmpfile = h5py.File(run+'Pulse_Amp'+amp+'_'+gain+'.hdf5', 'r')
      n_measurements = tmpfile.attrs["n_measurements"]
      attr_vals = defaultdict(list)
      meas_indicides = defaultdict(list)
      default_attrs = ['adc_freq', 'awg_freq', 'block_length', 'n_adcs', 'n_channels']
      #print(combos)
      cut_attrs = [attr for attr in tmpfile['Measurement_0'].attrs if attr not in default_attrs]
      print(cut_attrs)
      for meas in range(n_measurements):
        attr_key = ''
        for attr in cut_attrs:
          attr_val = tmpfile['Measurement_'+str(meas)].attrs[attr]
          if attr_val not in attr_vals[attr]: 
            attr_vals[attr].append(attr_val)
          attr_key += str(attr_val) + '-'
        attr_key = attr_key[:-1] #remove final "-"
        meas_indicides[attr_key].append(meas)
        #print(meas_indicides[attr_key])
      print(meas_indicides.keys())
      combos = list(meas_indicides.keys())
      for combo in combos:
        print(combo)
        measurements = meas_indicides[combo]
        #print(meas_indicides[combo_key])
        tmpdata = get_data_from_hdf5(tmpfile, measurements)
        data_full.update({combo:tmpdata})
        print("n_blocks: " + str(n_blocks) + ", block_length: " + str(block_length) + ", len(measurements)*n_blocks: " + str(len(measurements)*n_blocks))
        samples_to_save.update({combo:np.zeros(shape=(n_adcs, n_channels, len(measurements)*n_blocks, block_length))})
        bits_to_save.update({combo:np.zeros(shape=(n_adcs, n_channels, len(measurements)*n_blocks, block_length))})
        for adc in range(n_adcs):
          for channel in range(n_channels):
            data_tmp = {}
            data_tmp.update({combo:tmpdata[adc][channel]})
            print("Running analysis for ADC "+str(adc+1)+", Channel "+str(channel+1)+", Amplitude "+str(amp)+" ...")
        ...
        print(len(combos))
      save_blocks(saveFile, samples_to_save, bits_to_save, attr_vals, combos)
